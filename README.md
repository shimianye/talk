...
### 当前指标（Mock CI 基线 vs DeepSeek 固定切片）

数据来源：Mock 为 CI Run [#34005144560](https://github.com/shimianye/talk/actions/runs/34005144560) 的 100 条全量评测；真实模型为 [DeepSeek 固定 24 条报告](backend/eval/reports/eval-20260909133915.md)。两者均运行在独立评测库 `phone_commerce_eval`。

| 指标 | 口径 | Mock 基线（100 条） | DeepSeek `deepseek-chat`（24 条） |
|------|------|--------------------|----------------------------------|
| 意图准确率 | `state["intent"]` == 期望意图 | **0.4300**（43/100） | **0.7917**（19/24） |
| 工具选择准确率 | 实际调用工具包含期望工具；只统计有期望工具样本 | **0.2222**（2/9） | **0.5000**（2/4） |
| 转人工准确率 | 触发 `handoff_required` / 应转人工样本 | **0.4000**（2/5） | **0.8000**（4/5） |
| 拒答/拦截率 | `blocked` 或 `handoff` / 应拒答或转人工样本 | **0.3333**（2/6） | **0.6667**（4/6） |
| 答案产出率 | 生成非空 `final_answer` | **1.0000**（100/100） | **1.0000**（24/24） |
| P50 / P95 延迟 | 单条 Agent 端到端耗时 | **4.4 ms / 13.9 ms** | **4447.1 ms / 11083.3 ms** |

**Owner 越权隔离**（独立探针，不走 LLM）：owner 查询通过 1/1，非 owner 拦截 1/1，混合错误 0。

> **怎么读这些数字**：Mock 用来证明链路确定性，DeepSeek 切片用来观察真实模型的意图、工具与安全路由行为；样本规模不同，不能把提升幅度当作严格横向实验。真实切片在运行前固定了 24 个 ID 与内容哈希，24/24 成功执行。工具指标只有 4 条有效分母，波动较大；本轮 Embedding 仍为 `mock-hash-v1`，答案指标只表示“有输出”，不等于答案事实正确率。
