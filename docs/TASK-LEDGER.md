...
| T-N | 求职证据增强：真实模型评测、决策依据展示、评测可视化与面试材料 | Codex | 进行中 | T-L | 在 DeepSeek 全量真实评测基础上，将知识库 Embedding 从 Mock 切换为本地 Ollama `bge-m3`（1024 维），正在执行全量重建、数据库核验与真实模型复测；原 24 条切片仅保留为小样本历史 |
