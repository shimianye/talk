...
| T-N | 求职证据增强：真实模型小样本评测、决策依据展示、评测可视化与面试材料 | Codex | 待复核 | T-L | 决策摘要、知识来源汇总、真实工具计数、评测指标卡、固定 24 条切片与面试材料已完成；47 passed / 3 skipped，前端构建通过；真实模型报告因 Docker Desktop 4.80 启动时 `dockerInference` reparse point 崩溃而未执行，未产生外部模型调用或虚构数字 |
