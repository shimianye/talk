...
| ID | 任务 | 负责方 | 状态 | 依赖 |
|----|------|--------|------|------|
| T-H | 前端 SSE 流式消费（改 `frontend/src/api.ts`） | Codex | 待复核 | 阶段一完成 | 方案 A 已实现；含跨 chunk SSE 解析、AbortController 清理、按帧 token 缓冲、done 收尾和同步降级；后端 SSE/LLM 不改；设计见 `docs/superpowers/specs/2026-09-06-t-h-sse-design.md`；commit `cacd648`，待 WorkBuddy 只读复核 |
