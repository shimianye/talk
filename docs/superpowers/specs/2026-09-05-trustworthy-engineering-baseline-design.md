...
当前锁定的 `pgvector==0.5.0` 不提供 `pgvector.alembic` 模块，因此 Alembic 不导入该不存在的扩展。模型中的 `pgvector.sqlalchemy.Vector` 参与 autogenerate，生成迁移显式导入 `pgvector.sqlalchemy`，并由实际升级和漂移检查验证 `Vector(1024)` 类型。在线和离线配置均启用 `compare_type=True` 与 `compare_server_default=True`。应用启动、测试和初始化脚本不再调用 `Base.metadata.create_all()`。
